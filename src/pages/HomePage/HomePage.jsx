...
import { thaiDate, selectBgColor } from '../../utils/dateTime';
...
class HomePage extends Component {
  ...
  onCreateEvent = (schedulerData, slotId, slotName, start, end, type, item) => {
    ...
    const filteredRoom = filter(this.props.rooms, ({ name }) => name === slotName)[0];
    const roomStatus = get(filteredRoom, 'type');
    const { status } = selectBgColor(start, end);

    console.log('slotName', split(slotName, ' ')[0]);
    if (isOverlap) {
    ...
    } else if(roomStatus !== 'AFTERHOURS' && status === RESERVATION_STATUS_AFTER_HOUR) {
